...
// Sends an existing admin a "reset your password" link -- same underlying
// token mechanism and landing page as the invite email, just different
// copy so it reads correctly for someone who already has an account and
// forgot their password, rather than someone brand new. Never throws --
// returns true/false so the caller can respond the same way either way
// (never confirm or deny whether the email actually matched an account).
function send_admin_password_reset_email(array $config, string $email, string $name, string $token): bool {
    $link = "https://themorganlegacy.com/admin/auth/set_password.php?token={$token}";
    ...
    try {
        ...
        $mail->AddEmbeddedImage(__DIR__ . '/../assets/images/logo.png', 'logoimg');
        $mail->isHTML(true);
        $mail->Subject = "Reset your Morgan Legacy Scholarship admin password";
        $mail->Body = "
        <!DOCTYPE html>
        <html>
        <body style='margin:0; padding:0; background-color:#f2f2f5; font-family: Arial, Helvetica, sans-serif;'>
                        ...
                        <table role='presentation' width='600' cellpadding='0' cellspacing='0' style='max-width:600px; width:100%; background-color:#ffffff; border-radius:10px; overflow:hidden;'>
                            ...
                            <tr>
                                <td style='padding: 36px 40px 10px 40px; color:#212529; font-size:15px; line-height:1.7;'>
                                    <p style='margin:0 0 16px;'>Dear {$name},</p>
                                    <p style='margin:0 0 16px;'>
                                        We received a request to reset the password for your Morgan Legacy
                                        Scholarship admin account. Use the button below to choose a new one.
                                        If you didn't request this, you can safely ignore this email -- your
                                        password won't change unless you click through and set a new one.
                                    </p>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td style='padding: 0 40px 36px 40px; color:#212529; font-size:15px; line-height:1.7;'>
                                    <p style='margin:0 0 4px;'>This link is valid for 1 hour.</p>
                                    <p style='margin:0;'>The Morgan Legacy Scholarship</p>
                                </td>
                            </tr>
        ...
        error_log("send_admin_password_reset_email failed for {$email}: {$mail->ErrorInfo}");
        return false;
    }
}
