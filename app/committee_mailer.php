...
// Sends a newly-added committee member their confirmation link. Final
// Review can't be sent to them (see admin/send_committee_review.php) until
// they click through -- this confirms the email address is real and
// actually theirs before it's ever used to deliver a review link. Never
// throws -- returns true/false, meant to be checked by the caller since
// this email is what actually activates the roster entry, not just a
// best-effort follow-up. Also reused to resend the same confirmation if
// it's still pending.
function send_committee_confirmation_email(array $config, string $email, string $name, string $token): bool {
    $link = "https://themorganlegacy.com/committee/confirm.php?token={$token}";
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = "Confirm your spot on the Morgan Legacy Scholarship committee";
        $mail->Body = "
        <!DOCTYPE html>
        <html>
                        ...
                        <table role='presentation' width='600' cellpadding='0' cellspacing='0' style='max-width:600px; width:100%; background-color:#ffffff; border-radius:10px; overflow:hidden;'>
                            ...
                            <tr>
                                <td style='padding: 36px 40px 10px 40px; color:#212529; font-size:15px; line-height:1.7;'>
                                    ...
                                    <p style='margin:0 0 16px;'>
                                        You've been added to the Morgan Legacy Scholarship committee roster.
                                        Please confirm your email so we know it's really you -- Final Review
                                        applications can't be sent to you until you do.
                                    </p>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td style='padding: 0 40px 36px 40px; color:#212529; font-size:15px; line-height:1.7;'>
                                    <p style='margin:0 0 4px;'>Thank you for serving on the committee!</p>
        ...
        error_log("send_committee_confirmation_email failed for {$email}: {$mail->ErrorInfo}");
        return false;
    }
}
